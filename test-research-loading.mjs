...
research.slice(0, 3).forEach((doc, i) => {
  const fname = doc.source_file.substring(0, 40);
  console.log(`   [${i+1}] ${fname}${fname.length >= 40 ? '...' : ''}`);
  console.log(`       Sage: ${doc.sage_label || 'UNMATCHED'} (ID: ${doc.sage_id})`);
  console.log(`       Words: ${doc.word_count}, Confidence: ${(doc.match_confidence * 100).toFixed(0)}%`);
});
