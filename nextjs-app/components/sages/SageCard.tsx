...
export function SageCard({ sage, locale, onClose }: SageCardProps) {
  ...
  // Tagged with the sage/locale it was loaded for, so a late response for a
  // previous sage can never render under the current one.
  const researchKey = `${sage.id}:${locale}`
  const [research, setResearch] =
    useState<{ key: string; docs: ResearchDoc[]; isFallback: boolean } | null>(null)
  ...
  useEffect(() => {
    // data.json flags every sage that has a research file; skip the 404.
    if (sage.has_research === false) return
    let alive = true
    // The same static files the sage page renders (/research/<id>.json), not
    // the Supabase research_content table, which is stale and partly
    // misattributed. Localized research first (/research/<id>.<locale>.json,
    // generated by translate_research.py), then Hebrew as fallback.
    const fetchDocs = async (src: string): Promise<ResearchDoc[]> => {
      try {
    ...
    const load = async () => {
      if (locale !== 'he') {
        const docs = await fetchDocs(`/research/${sage.id}.${locale}.json`)
        if (docs.length) {
          if (alive) setResearch({ key: researchKey, docs, isFallback: false })
          return
        }
      }
      const docs = await fetchDocs(`/research/${sage.id}.json`)
      if (alive) setResearch({ key: researchKey, docs, isFallback: locale !== 'he' })
    }
    load()
    return () => { alive = false }
  }, [sage.id, sage.has_research, locale, researchKey])
  ...
  return (
    <article className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
        ...
        {/* Research documents, shown the way the sage page shows them */}
        {research?.key === researchKey && research.docs.length > 0 && (
          <ResearchSection
            key={researchKey}
            sageId={sage.id}
            locale={locale}
            initialDocs={research.docs}
            initialIsFallback={research.isFallback}
            collapsed
          />
        )}
