...
interface ResearchSectionProps {
  ...
  locale: Locale
  /**
   * Documents resolved on the server. When supplied, the text is rendered into
   * the initial HTML and no client fetch happens; the reading controls stay
...
export function ResearchSection({
  ...
  // Client fetch is only a fallback for callers that cannot resolve on the
  // server. The sage page passes initialDocs, so this effect no-ops there.
  useEffect(() => {
        ...
        {docs.map((doc, i) => (
          <details key={i} open={i === 0}
            ...
            <div
              ...
              // otherwise stretch the container past the viewport on mobile.
              className="px-4 pb-4 pt-1 text-sm text-ink-200 whitespace-pre-line break-words [overflow-wrap:anywhere] border-t border-ink-700/30"
