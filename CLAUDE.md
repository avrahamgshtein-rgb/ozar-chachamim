...
2. **`public/data.json`** (`{ nodes: Sage[], links: Connection[] }`) is the canonical dataset consumed by the app. Raw node fields (`era`, `era_key`, `era_label`, `central_idea`, `chapter_type`, comma-string `tags`) get remapped to the `Sage` type by `lib/dataFoundation.ts::normalizeSage`, shared by `lib/serverData.ts` (server) and `lib/appShellDataLoader.ts` (client). Both then copy the result into `Sage` objects through an explicit field list — those two lists must stay in sync if you add a field, or the field is dropped silently on one side.
...
### Client data loading: static pipeline only

`AppShell.tsx`'s bootstrap calls `loadAppShellData()`, which fetches `data.json`, the supplements and `data-patch.json` in parallel and merges them with the same `mergeDatasets`/`applyPatches` the server uses, so the home page and the sage pages always show the same corpus. It does **not** read Supabase. It used to fetch Supabase first (`sages_with_stats` / `connections_with_names`) and prefer it whenever it held ≥300 sages and ≥100 links. That made every visitor wait on Supabase, with no timeout, before anything rendered. And because production Supabase holds an old snapshot (missing ~100 current sages such as Rashi, and carrying links deleted since), that stale data won. Don't reintroduce a Supabase read for the public graph. Supabase is a separate, independently populated store, used for auth, chat and the personal area. The one leftover is `SearchBar`'s `searchSages` fallback, used only while the static data is still loading. The home page's initial header count and About stats come from `lib/serverData.ts::getCorpusStats()` at build time.
